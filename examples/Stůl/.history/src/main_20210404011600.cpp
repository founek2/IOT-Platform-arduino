...
bool CHECK_YAMAHA_ENABLED = true;
...
void setup()
{
    ...
    Node *nodeSwitch = plat.NewNode("relay", "Led stůl", "switch");
    propSwitch = nodeSwitch->NewProperty("power", "Led stůl", "boolean");
    ...
    switchL = nodeSwitch->NewProperty("left", "Levé", "boolean");
    ...
    switchR = nodeSwitch->NewProperty("right", "Pravé", "boolean");
    ...
    Node *reciever = plat.NewNode("reciever", "Reciever", "switch");
    recieverPower = reciever->NewProperty("power", "Zapnutí", "boolean");
    ...
    recieverVolume = reciever->NewProperty("volume", "Hlasitost", "integer");
    recieverVolume->setFormat("0:80");
    recieverVolume->setSettable(true);
    recieverVolume->setCallback([](Property *prop) {
        yamaha.setVolume(prop->getValue().toInt());
    });

    recieverInput = reciever->NewProperty("input", "Vstup", "enum");
    ...
    recieverInput->setCallback([](Property *prop) {
        yamaha.setInput(prop->getValue().c_str());
    });
    ...
    yamahaTicket.attach(5 * 60, []() { CHECK_YAMAHA_ENABLED = true; });

    switchL->setValue("false");
    switchR->setValue("false");
    propSwitch->setValue("false");

    // recieverPower->setValue("false");
    // recieverVolume->setValue("40");
    // recieverInput->setValue("net_radio");
}
...
void loop()
{
    ...
    if (recieverChange)
    {
        ...
        {
            yamaha.setInput(recieverInput->getValue());
        }
...
void syncYamaha()
{
    ...
    if (recieverPower->getValue() != yamaha.getPower())
        recieverPower->setValue(yamaha.getPower() == "on" ? "true" : "false");
...
void processStates()
{
    ...
    else if (!RELAY1_STATE && !RELAY2_STATE)
        propSwitch->setValue("false");
}
