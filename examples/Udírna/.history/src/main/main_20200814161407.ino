...
const char *API_KEY = "219321767bda4bc94a8aa0e2de07e7c7"; // tata udirna v2

IOTPlatforma plat(API_KEY);
const String keys[] = {"temp"};
...
void setup()
{
    ...
    tft.println("v2.IOTplatforma.cloud");
...
void sendTemperature()
{
    if (temperature > -20)
    {
        const float values[] = {temperature};
        bool success = plat.send(values, keys, 1);
        if (!success)
        {
