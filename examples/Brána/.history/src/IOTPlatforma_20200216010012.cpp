...
IOTPlatforma::IOTPlatforma(const char *apiKey, uint my_mode) : API_KEY(apiKey), wifiClient(), client(wifiClient), timer(1),mode(my_mode)
{
...
void IOTPlatforma::init()
{
    ...
    // this->sendEmptyAck();
    if (mode == IOTPlatforma::CONTROL_MODE)
            this->timer.add(0, 5 * 60 * 1000, [this](void) { this->shouldSendAck = true; }, nullptr, true);
}

void IOTPlatforma::sendAck() {
    this->shouldSendAck = true;
}
