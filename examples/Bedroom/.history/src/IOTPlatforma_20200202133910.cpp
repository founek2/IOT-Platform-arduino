...
void IOTPlatforma::getTopic()
{
    ...
    client.setCallback([this](char *topic, byte *payload, unsigned int length) { this->handleSubscribe(topic, payload, length); });
...
bool IOTPlatforma::downloadTopic(String &out, int len)
{
    ...
    if (https.begin(client, iot_api + "/api/iot/topic?API_KEY=" + API_KEY))
    { // HTTPS
...
bool IOTPlatforma::send(const float values[], const String keys[], const int count)
{
    ...
    Serial.println(json);
    return client.publish(deviceTopic.c_str(), json.c_str());
}
...
bool IOTPlatforma::sendInitControl(const int values[], const String keys[], const int count)
{
    ...
    Serial.println(json);
    return client.publish((deviceTopic + "/initControl").c_str(), json.c_str());
}
...
{
    return client.subscribe((deviceTopic + "/update").c_str(), 1);
}
...
void IOTPlatforma::handleSubscribe(char *topic, uint8_t *payload, unsigned int len)
{
    ...
    const String updateTopic = deviceTopic + "/update";
    if (strncmp(updateTopic.c_str(), topic, updateTopic.length()) != 0)
        defaultCallback(topic, payload, len);
