...
const char fingerprint[] = "CE:EF:7D:A2:87:30:E1:CD:AF:91:2C:B2:18:EF:72:51:02:35:AB:F9";
...
void IOTPlatforma::init()
{
    ...
    if (strncmp(data, "00/", 3) == 0)
    {
        Serial.print("topic is: ");
        Serial.println(data + 2);
        strncpy(topic, data + 2, maxLen);
    }
    else
    {
        Serial.println("Old values are: " + String(topic) + "," + String(topic));
...
bool IOTPlatforma::send(float values[], String keys[], int count)
{
    ...
    Serial.println(json);
    return client.publish((String(topic) + "/save").c_str(), json.c_str());
}